...
const mutations = {
  ...
  [types.SET_TAG_BOOKS] (state, {books, type}) {
    if (type !== 'merge') {
      ...
      state.tagBooks[state.currentBookTag] = [...state.tagBooks[state.currentBookTag], ...books]
    }
  },
  [types.SET_CURRENT_TAG_BOOKS] (state, {books, type}) {
    if (type !== 'merge') {
      ...
      state.currentTagBooks = [...state.currentTagBooks, ...books]
    }
  },
...
const actions = {
  getCurrentTagBooks ({commit, state}, {count = 10, start = 0, type}) {
    axios.get('/api/book/search', {
      params: {
        q: state.currentBookTag,
        count,
        start
      }
    })
      .then(res => {
        commit(types.SET_TAG_BOOKS, {books: res.data.books, type})
        commit(types.SET_CURRENT_TAG_BOOKS, {books: res.data.books, type})
      })
  },
  getTypeBooks ({commit, state}, {count = 10, start = 0}) {
    axios.get('/api/book/search', {
      ...
      .then(res => {
        commit(types.SET_TYPE_BOOKS, res.data.books)
        commit(types.SET_CURRENT_TYPE_BOOKS, res.data.books)
      })
  }
}
...
const getters = {
  ...
  currentBookTags: state => state.currentBookTags,
  currentBookTag: state => state.currentBookTag,
  tagBooks: state => state.tagBooks,
  currentTagBooks: state => state.currentTagBooks,
  bookTypes: state => state.bookTypes,
  typeBooks: state => state.typeBooks,
  currentBookType: state => state.currentBookType,
  currentTypeBooks: state => state.currentTypeBooks
}
